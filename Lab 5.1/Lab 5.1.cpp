...
*/
// Depending upon player actions throughout the game V can choose between different options to conduct the attack finish
	string p;
	...
	for (auto c : p) { // поиск самого длинного слова в строке
		...
		if ((c < 'a' or c > 'z') and (c < 'A' or c > 'Z') and ((p[q] >= 'a' and p[q] <= 'z') or (p[q] >= 'A' and p[q] <= 'Z'))) {
			temp_n++;
			s_temp = "";
		}

		if ((c >= 'a' and c <= 'z') or (c >= 'A' and c <= 'Z')) s_temp += c;
		if (temp_n == n) {
			if (size(s_temp) > size(s_dlinnoe)) s_dlinnoe = s_temp;
			s_temp = "";
	...
	for (int j = f - 1; j >= 0; j--) { // стираем наиболее длинные слова
		if (r[j] != 0)p.erase(r[j] - 1, siz[j] + 1);
		else p.erase(r[j], siz[j] + 1);
	}
